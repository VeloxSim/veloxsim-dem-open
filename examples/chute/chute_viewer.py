"""
Regenerate stl_animation.html from stl_results.json with particle subsampling.
Reduces file size so the browser can actually load it.
...
if not RESULTS.exists():
    print(f"ERROR: {RESULTS} not found"); sys.exit(1)

# --- Load results ---
print(f"Loading {RESULTS.name} ({RESULTS.stat().st_size/1024/1024:.0f} MB)...", flush=True)
...
frames = data["frames"]
R = config["radius"]
MAX_PARTICLES = config["n_particles"]
TONNAGE = config.get("tonnage_tph", 3000)

print(f"  {len(frames)} frames, max particles: {MAX_PARTICLES}")
...
for fr in frames[::step]:
    ...
    compact = {"t": round(fr["t"], 3), "n": n_sub, "p": [], "s": []}
    for i in indices:
        px, py, pz = fr["pos"][i]
        vx, vy, vz = fr["vel"][i]
...
print(f"  Subsampled: {len(anim_frames)} frames, max {max_n} particles/frame (1/{PARTICLE_SUBSAMPLE})")
...
print(f"  Payload: {len(payload)/1024/1024:.0f} MB")
...
<div id="info">
  ...
  <div><span class="c" style="background:#f87171"></span>__TPH__ tph, __D__mm particles</div>
...
for (const [name, data] of Object.entries(STL)) {
    ...
    if (isVirtual) {
        ...
        const mat = new THREE.LineDashedMaterial({ color, dashSize: 0.05, gapSize: 0.03, transparent: true, opacity: 0.7 });
...
html = html.replace("__TPH__", f"{TONNAGE:.0f}")
html = html.replace("__D__", f"{R*2*1000:.0f}")

out = OUT_DIR / "stl_animation.html"
print(f"Writing {out}...", end="", flush=True)
out.write_text(html, encoding="utf-8")
print(f" done ({out.stat().st_size/1024/1024:.0f} MB)")
